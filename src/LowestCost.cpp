...
void LowestCost::FillAdjMatrix(std::vector<Station> * stations,std::vector<Path>* path)
{
...
void LowestCost::PrintAdjMatrix(std::vector <Station>* stations)
{
    for (int i = 0; i < adjMatrix.size(); i++)
    {
        ...
        for (int j = 0; j < adjMatrix.size(); j++)
        {
            cout <<"firstST : "<<adjMatrix[i][j].getFirstST() << "  secondST : " << adjMatrix[i][j].getSecondST()
             << "   " << adjMatrix[i][j].getSubwayCost() << " , "<<adjMatrix[i][j].getTaxiCost()<<" , "<<
             adjMatrix[i][j].getBusCost()<<"     "<<adjMatrix[i][j].getSubway_Taxi_Line() 
